...
final class ChatPreferencesTests: XCTestCase {
    ...
    func testReducedKeepsADigestAsItsOwnRowAndBreaksTheRun() {
        ...
        XCTAssertEqual(rows[1].kind, .digest)
    }
    ...
    private func digest(_ id: String, at: Double = 1) -> Message {
        ...
        message.digest = WorkDigest(tools: [WorkDigest.ToolCount(name: "Bash", count: 2)])
        return message
    }
